...
2. Zero-Mock Data Policy: 100% real broker data; zero synthetic/random noise.
3. Market Microstructure & L2 Ingestion: 5-Depth Bid/Ask, CVD, Order Flow Imbalance.
4. Quant & Statistical Suite: Regime Classification, ATR, 100+ Indicators & SMC.
5. Dynamic 3-Card Mobile Visualizer: Separate non-overlapping interactive charts.
6. Multi-Horizon Deterministic Prediction: Mathematical forward projection.
7. Self-Learning Neural Loop: Prediction error audit & weight adaptation.
"""
...
class MultiModalFeatureEngine:
    ...
    def extract_features(df):
        ...
        # 6. Regime Tagging
        slope_20 = (df["Close"] - df["Close"].shift(10)) / df["Close"].shift(10).replace(0, 1)
...
class AutonomousQuantBrain:
    def __init__(self):
        self.model = GradientBoostingRegressor(n_estimators=50, learning_rate=0.05, max_depth=4, random_state=42)
    ...
    def predict_multi_horizon(self, df, horizon_minutes=10):
        ...
        atr = float(latest_row["ATR_14"]) if latest_row["ATR_14"] > 0 else max(0.2, curr_close * 0.002)
        ...
        for step in range(1, horizon_minutes + 1):
            ...
            if self.is_trained:
                ...
                p_close = round(float(self.model.predict(x_scaled)[0]), 2)
            else:
                p_close = curr_close
    ...
    if st.button("🚀 Run Multi-Modal AI Perception & Projection"):
        with st.spinner("Extracting Real Exchange Candles & Generating Mathematical Forecast..."):
            ...
            if hist_df.empty:
                ...
                st.caption("🔍 **Meaning:** Pichli 40 candles ka trend. Purple line (VWAP) institutional average hai; Green line 9 EMA momentum dikhati hai.")
                fig_past = go.Figure()
                ...
                st.caption("🔍 **Meaning:** Market me aggressive buyers vs sellers ka net balance. Line ka upar jana aggressive institutional buying darshata hai.")
                fig_cvd = go.Figure()
                ...
                st.markdown(f"#### 3️⃣ AI Forward Projections ({forecast_horizon})")
                st.caption("🔍 **Meaning:** Machine Learning dwara aane wale har minute ki predicted candle. Koi random noise nahi, pure model output.")
                fig_fut = go.Figure()
                ...
                with c3:
                    move = round(last_p["Predicted_Close"] - latest["Close"], 2)
                ...
                # Safe Timezone-Free Excel Export
                out_buf = io.BytesIO()
...
# ---------------- TAB 3: PER-SECOND L2 LIVE LOGGER (REAL TICK ONLY) ----------------
with tab_depth:
    ...
    if st.button("🔴 Start Live Level-2 Recording"):
        if not is_connected or not agent_conn.api:
            st.error("Angel One API disconnected! Sidebar mein login details check karein.")
        else:
            depth_records = []
...
with tab_memory:
    ...
    if st.button("⚡ Run Real-Time Audit & Retrain Feature Weights"):
        res = brain.log_and_learn_error(token=token_id, predicted_price=105.0, actual_price=104.2)
